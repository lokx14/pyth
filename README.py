...
class bookTicket:
    ...
    def insertScree_11 (cls, r, c, name, gender, phoneNo, status):
        try:
            connection = mysql.connector.connect (host='localhost',database='BookYourTicket',user='root',password='lokz@123')
            cursor = connection.cursor()
            mysql_insert_query= """INSERT INTO screen_11 (Row, Col, Name, Gender, PhoneNo, Status)
                                                      VALUES (s, es, es, s, %s, s) """
...
try:
    connection = mysql.connector.connect (host='localhost',database=' BookYourTicket',user='root',password='lokz@123')
    cursor = connection.cursor()
    sql_update_query="""Update statistics set NoOfTickets = (select count (status) from Screen1)"""
...
try:
    connection = mysql.connector.connect (host='localhost',database='BookYourTicket',user='root',password='lokz@123')
    cursor = connection.cursor()
    sql_update_query= """ Update statistics set NoofTickets = (select count (status) from sioujcreen2)"""
    cursor.execute(sql_update_query)
    connection.commit()
except mysql.connector.Error as error:
    print("Failed to update table record: {}". format (error))
...
def screen2 (cls, rows, cols):
    ...
    try:
        connection = mysql.connector.connect (host='localhost',database='BookYourTicket',user='root',password='lokz@123')
        sql_select_Query = "select Row, Col from screen 12"
        cursor = connection.cursor()
        cursor.execute(sql_select_Query)
        records = cursor.fetchall()
        print("Cinema :")
...
def statisticsDetail1 (cls):
      try:
          mySQLConnection= mysql.connector.connect (host='localhost',database=' BookYourTicket',user='root',password='lokz@123')
...
def display (cls):
     ...
     if choice == 2:
         ...
         if screenChoice ==2:
                      ...
                      gender = input ("enter the gender\n")
                      phoneNo= input ("Enter the mbl no\n")
                      bookTicket.insertScree_12 (r, c, name, gender, phoneNo, "Boc d")
     ...
     if choice ==4:
         ...
         if datachoice == 1:
             ...
             bookTicket.getUserDetaill (user1Row, user1Col)
         if datachoice == 2:
